...
class ExploringMinimax(GamePolicy):
    ...
    def choose(self, player):
        if np.random.random() < self.epsilon:
            self._mu = np.ones(player.num_action)/np.sum(np.ones(player.num_action))
...
class UCBMinimax(GamePolicy):
    ...
    def choose(self, player):
        #print(player.T)
        exploration = 2 * np.log(2 * player.T ** 2 * player.num_action * player.oppo_num_action) / player.action_attempts
...
class EXP3(GamePolicy):
    ...
    def choose(self, advplayer):
        ...
        gammat = min(np.sqrt(advplayer.num_action * np.log(advplayer.num_action) / advplayer.t), 1)
        #print('gammat {}'.format(gammat))
        rhot = np.sqrt(2 * np.log(advplayer.num_action) / max(advplayer.t, 1) * advplayer.num_action)
        #print('rhot {}'.format(rhot))
        wt = np.exp(rhot * advplayer.cumulative_estimates) / \
            np.sum(np.exp(rhot * advplayer.cumulative_estimates))
        self._mu = gammat / advplayer.num_action * np.ones(advplayer.num_action) + (1 - gammat) * wt
        cdf = np.cumsum(self._mu)
        print(self._mu)
        no = np.random.random()
...
class OFULinMat(GamePolicy):
    ...
    def generatemu(self, contextualplayer):
        self.delta = 1 / (contextualplayer.T * 100)
        ucbgame = contextualplayer.game_estimates
        betak = np.sqrt(2 * np.log(np.sqrt(np.linalg.det(contextualplayer.V)) / (self.delta * contextualplayer.lamb ** 0.5))) + contextualplayer.lamb ** 0.5 * B
