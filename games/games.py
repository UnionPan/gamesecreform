...
class ContextualGame(RandomGame):
    ...
    def __init__(self, n1, n2, theta, T):
        super.__init__(RandomGame).__init__()
        self.theta = theta
        self.n1 = n1
        self.n2 = n2
        self._true_game = np.zeros((self.n1, self.n2))
    ...
    def reset(self):
        self._expert_games = np.random.uniform(size = (len(self.theta), self.n1, self.n2))
        self._true_game = np.sum([self.theta[i] * self.expert.games[i]
