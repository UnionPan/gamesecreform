...
from bandits.agents import BetaAgent
    ...
    def plot_beliefs(self):
        ...
        for i, agent in enumerate(self.agents):
            if type(agent) is not BetaAgent:
                ...
                y = np.array([stats.beta.pdf(x, a, b) for a, b in zip(agent.alpha, agent.beta)])
            ...
            for i, ax in enumerate(axes):
                ...
                if i == int(cols / 2):
                    title = '{}-arm Bandit - Agent Estimates'.format(self.bandit.k)
                    ax.set_title(title)
                if i == min_p:
